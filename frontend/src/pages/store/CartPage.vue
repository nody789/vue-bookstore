...
<template>
  <div class="max-w-2xl mx-auto">
    <h1 class="font-bold text-2xl text-stone-800 mb-6">購物車</h1>

    <div v-if="cart.items.length === 0" class="text-center py-20">
      <p class="text-stone-400 mb-4">購物車是空的</p>
    ...
    <div v-else>
      ...
      <div class="space-y-4">
        <div v-for="item in cart.items" :key="item.id"
          ...
          <!-- 書籍資訊 -->
          <div class="flex-1">
            <p class="font-medium text-stone-800 text-sm">{{ item.book.title }}</p>
            <p class="text-stone-400 text-xs">{{ item.book.author }}</p>
            <p class="text-amber-700 font-bold mt-1">NT$ {{ item.book.price.toLocaleString() }}</p>
          </div>
          <!-- 數量控制 -->
          <div class="flex items-center border border-gray-300 rounded overflow-hidden text-sm">
            <button @click="handleUpdate(item.id, item.quantity - 1)" :disabled="item.quantity <= 1"
              class="px-2 py-1 hover:bg-gray-50 disabled:opacity-40">−</button>
            <span class="px-3 py-1">{{ item.quantity }}</span>
            <button @click="handleUpdate(item.id, item.quantity + 1)" :disabled="item.quantity >= item.book.stock"
              class="px-2 py-1 hover:bg-gray-50 disabled:opacity-40">+</button>
          </div>
          <!-- 小計 -->
          <p class="text-stone-700 font-medium w-20 text-right text-sm">
      ...
      <div class="bg-white border border-gray-200 rounded-lg p-4 mt-4">
        <div class="flex justify-between text-stone-700 mb-4">
