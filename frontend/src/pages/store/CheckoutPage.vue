...
<template>
  <div class="max-w-lg mx-auto">
    <h1 class="font-bold text-2xl text-stone-800 mb-6">結帳</h1>

    <!-- 訂單摘要 -->
    <div class="bg-white border border-gray-200 rounded-xl p-5 mb-5 shadow-sm">
      <h2 class="font-semibold text-stone-700 mb-3 text-sm">訂單摘要</h2>
      <div class="space-y-1.5 text-sm text-stone-600">
        <div class="flex justify-between">
          <span>商品小計（{{ cart.itemCount }} 件）</span>
          <span>NT$ {{ cart.total.toLocaleString() }}</span>
        </div>
        <div v-if="couponResult" class="flex justify-between text-emerald-600">
          <span>折價券折扣（{{ couponResult.coupon.code }}）</span>
          <span>－ NT$ {{ couponResult.discountAmount.toLocaleString() }}</span>
        </div>
        <div class="flex justify-between">
          <span>運費</span>
          <span :class="cart.total >= 599 ? 'text-emerald-600' : ''">
            {{ cart.total >= 599 ? '免費' : 'NT$ 60' }}
          </span>
        </div>
        <div class="border-t border-gray-100 pt-2 mt-2 flex justify-between font-bold text-stone-800 text-base">
          <span>應付金額</span>
          <span class="text-amber-700">NT$ {{ finalTotal.toLocaleString() }}</span>
        </div>
      </div>
    </div>
    ...
    <!-- 折價券 -->
    <div class="bg-white border border-gray-200 rounded-xl p-5 mb-5 shadow-sm">
      <h2 class="font-semibold text-stone-700 mb-3 text-sm">折價券</h2>

      <!-- 已套用 -->
      <div v-if="couponResult"
        class="flex items-center justify-between bg-emerald-50 border border-emerald-200 rounded-lg px-4 py-3">
        <div>
          <p class="text-sm font-semibold text-emerald-700">{{ couponResult.coupon.code }}</p>
        ...
        <button @click="removeCoupon" class="text-stone-400 hover:text-red-500 transition text-sm">✕ 移除</button>
      </div>

      <!-- 輸入框 -->
      <div v-else class="flex gap-2">
        <input v-model="couponCode" type="text" placeholder="輸入折價券代碼" @keyup.enter="applyCoupon"
          class="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm uppercase focus:outline-none focus:ring-2 focus:ring-amber-500" />
      ...
      <p class="text-stone-400 text-xs mt-2">試試：WELCOME10 / SAVE100 / SUMMER20 / VIP15</p>
    </div>

    <!-- 收件資訊 -->
    <div class="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
      <h2 class="font-semibold text-stone-700 mb-4 text-sm">收件資訊</h2>
      <form @submit.prevent="submit" class="space-y-4">
        <div>
          <label class="block text-sm font-medium text-stone-700 mb-1">收件人姓名</label>
          <input v-model="form.recipientName" type="text" required
            ...
            :class="errors['recipientName'] ? 'border-red-400' : 'border-gray-300'" />
        ...
        <div>
          ...
          <input v-model="form.recipientPhone" type="tel" required placeholder="09xxxxxxxx"
            class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500"
            :class="errors['recipientPhone'] ? 'border-red-400' : 'border-gray-300'" />
          <p v-if="errors['recipientPhone']" class="text-red-500 text-xs mt-1">{{ errors['recipientPhone'] }}</p>
        </div>
        <div>
          <label class="block text-sm font-medium text-stone-700 mb-1">收件地址</label>
          <input v-model="form.shippingAddress" type="text" required
            class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500"
            :class="errors['shippingAddress'] ? 'border-red-400' : 'border-gray-300'" />
